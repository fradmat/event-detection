...
#normalizes and returns normalization factors (means)
#careful, this method modifies data by reference and does not return a copy!
def normalize_signals_whole_set(list_of_shots):
    print('normalizing across the entire set')
    pds = []
    ips = []
    dmls = []
    firs = []
    for df in list_of_shots:
        pds.extend(df.PD.values)
        ips.extend(df.IP.values)
        dmls.extend(df.DML.values)
        firs.extend(df.FIR.values)
        # print(df.FIR.values)
        
    # pds = np.asarray(pds)
    # print(pds.shape)
    mean_firs = np.mean(firs)
    mean_pds = np.mean(pds)
    mean_ips = np.mean(ips)
    mean_dmls = np.mean(dmls)
    
    for df in list_of_shots:
        df['FIR'] = df.FIR.values/mean_firs
        df['PD'] = df.PD.values/mean_pds
        df['DML'] = df.DML.values/mean_dmls
        df['IP'] = df.IP.values/mean_ips
        # print(df.FIR.values)
    return {'mean_firs': mean_firs, 'mean_pds':mean_pds, 'mean_dmls':mean_dmls, 'mean_ips':mean_ips}

def normalize_signals_with_factors(df, factors):
    df['FIR'] = df.FIR.values/factors['mean_firs']
    df['PD'] = df.PD.values/factors['mean_pds']
    df['DML'] = df.DML.values/factors['mean_dmls']
    df['IP'] = df.IP.values/factors['mean_ips']
    return df
